...
public class ChemicalElementSeeder implements DataSeeder {
    ...
    private static final Set<String> FULL_KIT_ELEMENT_SYMBOLS = Set.of(
            "Al",
            "Ca",
            "C",
            "Cu",
    ...
    public void seed() {
        for (String[] e : ELEMENTS) {
            ...
            String symbol = e[1];
            String name = e[2];
            String periodicCategory = e[3];
            BigDecimal atomicMass = new BigDecimal(e[4]);
            Integer period = Integer.valueOf(e[5]);
            Integer groupNumber = parseNullableInteger(e[6]);

            ChemicalSubstance substance = chemicalSubstanceRepository
                    .findByFormula(symbol)
                    .orElseGet(ChemicalSubstance::new);

            substance.setFormula(symbol);
            substance.setName(name);
            substance.setVietnameseName(null);
            substance.setType(ChemicalSubstanceType.ELEMENT);
            substance.setChemicalGroup(mapElementCategoryToChemicalGroup(periodicCategory));
            substance.setState(SubstanceState.UNKNOWN);
            substance.setMolarMass(atomicMass);
            substance.setActive(true);

            /*
             * Mặc định 118 nguyên tố không tự nằm trong hộp full kit.
             * Chất nào thật sự có trong hộp thì sau này set true bằng file compound/kit seed hoặc admin.
             */
            substance.setIncludedInFullKit(FULL_KIT_ELEMENT_SYMBOLS.contains(symbol));

            if (substance.getDescription() == null) {
                substance.setDescription(null);
            }

            if (substance.getSafetyNote() == null) {
                substance.setSafetyNote(null);
            }

            ChemicalSubstance savedSubstance = chemicalSubstanceRepository.save(substance);

            ElementDetail detail = elementDetailRepository
                    .findBySubstance_Id(savedSubstance.getId())
                    .orElseGet(ElementDetail::new);

            detail.setSubstance(savedSubstance);
            detail.setAtomicNumber(atomicNumber);
            detail.setSymbol(symbol);
            detail.setPeriodicCategory(periodicCategory);
            detail.setAtomicMass(atomicMass);
            detail.setPeriod(period);
            detail.setGroupNumber(groupNumber);

            elementDetailRepository.save(detail);
        }
    }
    ...
    private ChemicalGroup mapElementCategoryToChemicalGroup(String category) {
        if (category == null) {
            return ChemicalGroup.OTHER;
        }

        String value = category.toLowerCase();

        if (value.contains("noble gas")) {
            return ChemicalGroup.GAS;
        }

        if (value.contains("metal")) {
            return ChemicalGroup.METAL;
        }

        if (value.contains("nonmetal")) {
            return ChemicalGroup.NONMETAL;
        }
